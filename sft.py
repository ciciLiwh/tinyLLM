...
class SFTDataset(Dataset):
    ...
    def __init__(self, path: Path, tokenizer: ByteLevelBPETokenizer, eos_id: int) -> None:
        super().__init__()
        if not path.exists():
            raise FileNotFoundError(f"SFT file not found: {path}. Expect jsonl with fields instruction/input/output.")
...
def collate_fn(
    ...
    for ids in batch:
        trimmed = ids[: max_seq_len - 1]
        seq = torch.tensor(trimmed + [pad_id], dtype=torch.long)
        input_seqs.append(seq[:-1])
        target = torch.tensor(trimmed + [pad_id], dtype=torch.long)
        target_seqs.append(target[1:])
    inputs = pad_sequence(input_seqs, batch_first=True, padding_value=pad_id)
    targets = pad_sequence(target_seqs, batch_first=True, padding_value=ignore_index)
    return inputs, targets


def load_model(ckpt_path: Path, device: torch.device) -> Tuple[DecoderOnlyTransformer, int]:
    if not ckpt_path.exists():
        raise FileNotFoundError(f"Base checkpoint not found at {ckpt_path}. Train with train.py first.")
...
def main() -> None:
    ...
    train_loader = DataLoader(
        ...
        shuffle=True,
        collate_fn=lambda b: collate_fn(b, MAX_SEQ_LEN, eos_id, IGNORE_INDEX),
    )
    ...
    for epoch in range(NUM_EPOCHS):
        ...
        val_loss = evaluate(model, train_loader, device, vocab_size, IGNORE_INDEX)
        ppl = math.exp(val_loss) if val_loss < 20 else float("inf")
        print(f"epoch {epoch + 1} avg loss {val_loss:.4f}, ppl {ppl:.2f}")

        ckpt_path = OUT_DIR / f"sft_epoch{epoch + 1}.pt"
        torch.save(
            {
